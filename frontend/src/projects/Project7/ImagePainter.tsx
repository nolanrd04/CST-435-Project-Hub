...
import { project7API } from './api.ts';
...
function ImagePainter() {
  ...
  const [numInferenceSteps, setNumInferenceSteps] = useState<number>(1000);
  ...
  const resizeImageTo64x64 = (img: HTMLImageElement): string => {
  ...
  const processFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        const resized = resizeImageTo64x64(img);
  ...
  const handleColorize = async () => {
    ...
    try {
      const response = await project7API.colorizeImage({
      ...
      if (response.success) {
        ...
        setInferenceTime(response.inference_time_seconds);
        setSuccess(`Successfully colorized image in ${response.inference_time_seconds.toFixed(2)}s!`);
      } else {
    ...
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        <div style={{ fontSize: '14px', color: '#666', marginBottom: '8px' }}>
          Lower steps = faster but lower quality. 1000 steps = best quality (slow).
        </div>
        <input
          type="range"
          min="50"
      ...
      <div style={{ marginBottom: '30px' }}>
        ...
        >
          ...
          <div style={{ fontSize: '14px', color: '#999' }}>
            Image will be automatically resized to 64x64 and converted to grayscale
          </div>
        </div>
        <input
          ref={fileInputRef}
      ...
      {(uploadedImage || grayscalePreview || colorizedImage) && (
        <div style={{ marginBottom: '30px' }}>
          ...
          >
            ...
            {uploadedImage && (
              <div>
                <h4 style={{ marginBottom: '10px', color: '#333' }}>Original (64x64)</h4>
                <div
                  style={{
                    background: '#f5f5f5',
                    borderRadius: '8px',
